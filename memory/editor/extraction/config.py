...
from memory.editor.page_id import EXISTING_PAGE_ID_MAX


@dataclass(frozen=True)
class MemoryExtractionConfig:
    """限制受控 ReAct、关系扩展和候选审查的资源消耗。"""
    ...
    def __post_init__(self) -> None:
        ...
        if (
            isinstance(self.max_candidate_regenerations, bool)
            or not isinstance(self.max_candidate_regenerations, int)
            or not 0 <= self.max_candidate_regenerations <= 3
        ):
            raise ValueError("max_candidate_regenerations must be between zero and three")
