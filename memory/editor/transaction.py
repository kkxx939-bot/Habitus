...
class MemoryCommitTransaction:
            ...
            with ExitStack() as stack:
                ...
                with self.path_lock.fenced(guards):
                    ...
                    if all(state == "after" for state in states):
                        ...
                        raise MemoryCommitRecoveryError("prepared transaction overlaps an unknown later document state")
                    self.journal.mark(
    ...
    def _assert_current(self, expected: MemorySnapshotBatch) -> None:
        current = self.snapshot_reader.read_many(snapshot.identity for snapshot in expected.snapshots)
    ...
    def _verify_committed(
        ...
        current = self.snapshot_reader.read_many(snapshot.identity for snapshot in plan.read_set.snapshots)
    ...
    def _entry_state(self, entry: MemoryTransactionJournalEntry) -> str:
        snapshot = self.snapshot_reader.read(entry.uri)
