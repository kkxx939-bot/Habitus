...
from integrations.http_api.auth import HTTPAuthenticationError
...
ERROR_CODE_TO_HTTP_STATUS: Mapping[HTTPErrorCode, int] = MappingProxyType(
    {
        ...
        HTTPErrorCode.UNAUTHENTICATED: 401,
        HTTPErrorCode.PERMISSION_DENIED: 403,
...
def map_exception(exc: Exception) -> HTTPErrorSpec:
    ...
    if isinstance(exc, HTTPAuthenticationError):
        return HTTPErrorSpec(HTTPErrorCode.UNAUTHENTICATED, "A valid Bearer token is required", False)
...
def _framework_error_spec(status_code: int, message: str) -> HTTPErrorSpec:
    mapping = {
        ...
        401: (HTTPErrorCode.UNAUTHENTICATED, False),
        403: (HTTPErrorCode.PERMISSION_DENIED, False),
...
async def unhandled_error(request: Request, exc: Exception) -> JSONResponse:
    ...
    headers = {"WWW-Authenticate": "Bearer"} if isinstance(exc, HTTPAuthenticationError) else None
    return _response_from_spec(request, spec, headers=headers)
