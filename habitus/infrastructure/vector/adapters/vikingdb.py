...
from habitus.infrastructure.vector.factory import VectorStoreBuildContext, VectorStoreFactory
...
class VikingDBBackend:
    ...
    def _validate_collection_metadata(
        ...
        *,
        ...
        fields = {str(item.get("FieldName")): item for item in cast(list[Mapping[str, object]], raw_fields)}
        required = set(_FIELD_TYPES)
        missing = sorted(required - set(fields))
        if missing:
            raise VectorStoreIntegrityError(f"VikingDB collection schema is missing fields: {missing}")
        incompatible = sorted(
            name for name, expected_type in _FIELD_TYPES.items() if fields[name].get("FieldType") != expected_type
        )
        if incompatible:
            raise VectorStoreIntegrityError(f"VikingDB collection fields have incompatible types: {incompatible}")
        primary = fields["id"]
    ...
    async def _wait_full_index(self, records: Sequence[VectorStoreRecord]) -> None:
        expected = {_point_id(self._scope, record.identity): record.content_digest for record in records}
    ...
    async def _wait_incremental_index(
        ...
        expected = {_point_id(self._scope, record.identity): record.content_digest for record in upserts}
    ...
    async def _assert_incremental_scan_orders(
        ...
        for batch in _batches(upserts, 4999):
            expected = {_scan_order(record.identity): _point_id(self._scope, record.identity) for record in batch}
            response = await self._client.data(
                "/api/vikingdb/data/search/scalar",
                {
                    **self._data_identity(include_index=True),
...
def _item_from_record(record: VectorStoreRecord, *, scope: str) -> dict[str, object]:
    attributes = {name: list(value) if isinstance(value, tuple) else value for name, value in record.attributes.items()}
    return {
...
def _record_from_item(item: Mapping[str, object], *, scope: str) -> VectorStoreRecord:
    ...
    for field_name, expected in expected_index_fields.items():
        if fields.get(field_name) != expected:
            raise VectorStoreIntegrityError(f"VikingDB indexed field does not match attributes_json: {field_name}")
...
def _compile_filter(filters: VectorStoreFilter) -> dict[str, object]:
    clauses: list[dict[str, object]] = [{"op": "must", "field": "record_type", "conds": [_RECORD_TYPE_MEMORY]}]
