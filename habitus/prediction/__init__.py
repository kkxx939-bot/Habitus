...
# - **日历修正（判决层必做；树零语义不学日历）**：树的周几是机械键，学不了节假日与中国调休
#   ——国庆调休的周六按"周六"的统计预测（睡懒觉），实际是工作日（7:00 出门），整天系统性错；
...
#   降档闭嘴。原料全在（累积率 × 当日实况），零 LLM。例：出差日早上三连缺（吃药/晨跑/做早饭）
#   → 偏离度即已拉满 → 后续全天不再逐格报缺失。
...
from habitus.prediction.builder import build, config_digest
