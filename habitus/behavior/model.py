...
#
# ── 待定与悬置 ────────────────────────────────────────────────────────────────────────
# - TODO(BHV-FEEDBACK-001)（用户 2026-09-01 口述的闭环，记在这里等预测算法成形）：**规律性/
#   被约束性行为的预测必须是能动态调整的，不能是一个写死的固定数值。** 完整环路是——
#     观测到行为事件 → 形成数据沉淀 → 预测
#       ① 预测失败（没有按预测的行为执行）→ 数据执行回测或者调整
#       ② 预测成功 → 数据累积或者调整 → 最终走到主动提醒 / 代劳
#   现状**只有上半环**：树每夜从历史行为重建，预测记录只记"预测了什么"，后来实际发生了什么、
#   主体接受还是拒绝，全部丢弃——环合不上，那个"调整"就无从发生，数值只能靠重建时的历史统计
#   自然漂移，而不是被结果推着走。
#   - 具体场景：每周二、四 20:30 打球。周二 20:00 系统提醒 → 主体确认去了（②，这一次是证据，
#     累积）；某周二主体拒绝"今天不去"（①，这条**拒绝本身是可观测行为**，须走正门进树）；
#     连续几次拒绝 → 该行为的趋势下行 → 提醒降档直至闭嘴；反过来长期稳定确认 → 晋升到代劳档。
#   - 需要的东西（都还不存在）：结算账本（预测了什么 / 实际发生了什么 / 主体接受还是拒绝 /
#     何时提醒过）；数字拒绝走正门所需的观测 modality 交互模态（已列在上游契约缺口里）；
#     预测层的晋升判据（建议确认史）。
#   - 三条已裁定、不得违反的边界：① 行为树只有"观测→融合→归约"**一个写入口**，预测层永不回写
#     树，结算账本只存关联；② 拒绝提醒是主体的真实行为，必须进树，不能只留在预测侧的账本里；
#     ③ **干预混淆必须在提醒功能上线之前就位**——被提醒之后的发生不得进入自然率的估计，事后
#     无法把两种数据分开（``prediction/source.py`` 现在读到 ``reminded=True`` 直接硬拒，就是
#     这条护栏；字段已就位但恒为 False）。
#   - 影响大小：大，且**时间不可逆**。不做，闭环的下半环不存在、预测无法被结果修正；做晚了
#     （提醒已上线才补），那段时间的数据永久无法区分"自然发生"与"被提醒后发生"。
#   - 与既有条目的关系：``TODO(BHV-EPISODE-002)`` 记的是走完这个闭环之后**成熟行为**的历程留痕，
#     是本条的下游；预测侧的对应设计见 ``prediction/__init__.py`` 的 ``TODO(PRED-DOWNSTREAM-001)``
#     （反馈闭环、三档执行、干预混淆三节）。
