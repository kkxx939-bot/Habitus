...
from habitus.benchmark.context import select_answer_context
from habitus.benchmark.isolation import isolated_config
from habitus.benchmark.metrics import evidence_recall
...
from habitus.benchmark.prompts import answer_prompt
from habitus.benchmark.protocol import OPENVIKING_REFERENCE_REVISION
...
from habitus.model_client import ChatCallContext, ChatMessage, ChatRequest
...
def _source_revision() -> str:
    # 本文件是 habitus/benchmark/runner.py，parents[2] 才是仓库根。
...
def _source_tree_sha256() -> str:
    ...
    for name in (
        ...
        directory = root / name
        if directory.exists():
            benchmark_results = root / "habitus" / "benchmark" / "results"
            files.extend(
                path
