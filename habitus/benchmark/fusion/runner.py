...
from habitus.benchmark.fusion.dataset import FusionCase, FusionFragment
    ...
    *,
    ...
    for index, fragments in enumerate(case.segments, start=1):
        ...
        for item in derived:
            ...
            positions = {
                observation.observation_id: order for order, observation in enumerate(segment.fragments, start=1)
            }
            if item.behavior is None:
                record.unreadable_fragments.extend(positions[observation_id] for observation_id in item.observation_ids)
