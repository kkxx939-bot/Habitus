...
def __getattr__(name: str) -> Any:
    ...
    if name in {"ERROR_CODE_TO_HTTP_STATUS", "documented_error_responses"}:
        from habitus.integrations.http_api import errors

        return getattr(errors, name)
    if name in {"REQUEST_ID_HEADER", "current_request_id"}:
        from habitus.integrations.http_api import request_id

        return getattr(request_id, name)
    if name == "HTTPErrorCode":
        from habitus.integrations.http_api.schemas import HTTPErrorCode
