...
from habitus.model_client.json_parser import JSONParseMode, parse_json_response
...
class StructuredChatClient:
    ...
    def complete_json(
        ...
        *,
        ...
        for attempt in range(self.validation_retries + 1):
            response = self.client.complete(prepared, context=context)
            try:
    ...
    async def complete_json_async(
        ...
        *,
        ...
        last_error: _StructuredValidationFailure | None = None
        for attempt in range(self.validation_retries + 1):
            response = await self.client.complete_async(prepared, context=context)
            try:
                return self._validate_response(
                    response,
                    schema=schema,
    ...
    def _prepare(
        ...
        *,
        ...
        instruction_text = (
            "Return exactly one JSON value that satisfies the supplied native JSON Schema. "
            "Do not add Markdown fences, commentary, or undeclared fields."
            if self.client.config.structured_output_mode == "json_schema"
            else (
