...
from habitus.memory.conversation.ingress import (
...
from habitus.pre.conversation.messages.model import (
    ConversationBatch,
    ConversationMessage,
...
class ConversationJournalConfig:
    ...
    max_ingress_receipts: int = 100_000
    max_ingress_receipt_bytes: int = 16 * 1024
    ...
    def __post_init__(self) -> None:
        for name, value, maximum in (
            ...
            ("max_ingress_receipts", self.max_ingress_receipts, 1_000_000),
            ("max_ingress_receipt_bytes", self.max_ingress_receipt_bytes, 1024 * 1024),
...
class ConversationAppendResult:
    ...
    def __post_init__(self) -> None:
        if isinstance(self.appended_count, bool) or not isinstance(self.appended_count, int) or self.appended_count < 0:
...
class ConversationMessageJournal:
    ...
    def __init__(
        ...
        *,
        ...
        resolved_config = config or ConversationJournalConfig()
        self.layout = ConversationLayout(
            root,
            max_conversation_tree_entries=resolved_config.max_conversation_tree_entries,
        )
        self.path_lock = path_lock
        self.config = resolved_config
        self.ingress_receipts = ConversationIngressReceiptStore(
            self.layout,
            max_files=resolved_config.max_ingress_receipts,
            max_file_bytes=resolved_config.max_ingress_receipt_bytes,
        )
    ...
    def append(
        ...
        *,
        ...
        if ingress is not None and not isinstance(ingress, ConversationIngressRequest):
            raise TypeError("ingress must be ConversationIngressRequest or None")
        with self.path_lock.acquire(
            ...
            with guard.fenced():
                ...
                prepared_receipt: ConversationIngressReceipt | None = None
                if ingress is not None:
                    existing_receipt = self._read_ingress(address, ingress.delivery_id)
                    if existing_receipt is not None and existing_receipt.state is ConversationIngressState.PREPARED:
                        existing_receipt = self._recover_ingress_receipt(
                            address,
                            existing_receipt,
                    ...
                    if existing_receipt is not None:
                        self._require_matching_ingress(existing_receipt, ingress, batch)
                        if existing_receipt.state is ConversationIngressState.COMMITTED:
                            assert existing_receipt.next_sequence is not None
                            return ConversationAppendResult(
                                status=ConversationAppendStatus.UNCHANGED,
                                appended_count=0,
                                live=self._batch_or_none(state.conversation_id, live_messages),
                                next_sequence=existing_receipt.next_sequence,
                            )
                    prepared_receipt = self._prepare_ingress(address, ingress, batch)
                ...
                if not unseen:
                    if prepared_receipt is not None:
                        self._commit_ingress(address, prepared_receipt)
                    return ConversationAppendResult(
                        status=ConversationAppendStatus.UNCHANGED,
                        appended_count=0,
                        live=self._batch_or_none(state.conversation_id, live_messages),
                ...
                return ConversationAppendResult(
                    ...
                    live=ConversationBatch(state.conversation_id, updated_live),
    ...
    def read_live(self, address: ConversationAddress) -> ConversationBatch | None:
        ...
        with self.path_lock.acquire(
            ...
            with guard.fenced():
                ...
                live_messages = self._recover_archived_prefix(address, latest, live_messages)
                self._require_tail_continuity(state, live_messages)
                return self._batch_or_none(state.conversation_id, live_messages)
    ...
    def _prepare_seal(
        ...
        live_messages = self._recover_archived_prefix(address, latest, live_messages)
        self._require_tail_continuity(state, live_messages)
        self._recover_all_prepared_ingress(
            address,
            references=references,
            live_messages=live_messages,
            state=state,
    ...
    def _publish_seal(
        ...
        return ConversationSealResult(
            ...
            live=self._batch_or_none(plan.segment.conversation_id, plan.retained_messages),
        )
    ...
    def read_segment(
        ...
        stored_state = self._read_state_file(address)
        conversation_id = (
            stored_state.conversation_id if stored_state is not None else self._source_conversation_id(address)
        )
        segment = ConversationSegment(conversation_id, segment_id, messages)
    ...
    def read_editor_segment(
        self,
        address: ConversationAddress,
        source: ConversationSegment,
    ) -> ConversationSegment | None:
        """在完整轮次边界重组编辑输入；未闭合轮次返回显式延迟。"""

        if not isinstance(address, ConversationAddress):
            raise TypeError("address must be ConversationAddress")
        if not isinstance(source, ConversationSegment):
            raise TypeError("source must be ConversationSegment")
        history = self.list_history(address)
        positions = [index for index, segment in enumerate(history) if segment.segment_id == source.segment_id]
        if len(positions) != 1:
            raise ConversationJournalError("editor source segment is not retained exactly once")
        position = positions[0]
        current = history[position]
        if current.digest != source.digest:
            raise ConversationJournalError("editor source segment digest changed")
        if current.ends_mid_turn:
            return None

        start = position
        while history[start].starts_mid_turn:
            if start == 0:
                raise ConversationJournalError("editor input lost the retained beginning of its logical turn")
            previous = history[start - 1]
            following = history[start]
            if previous.end_sequence + 1 != following.start_sequence:
                raise ConversationJournalError("editor input history contains a sequence gap")
            if not previous.ends_mid_turn:
                raise ConversationJournalError("editor input turn-boundary flags are inconsistent")
            start -= 1

        selected = history[start : position + 1]
        for previous, following in zip(selected, selected[1:], strict=False):
            if previous.end_sequence + 1 != following.start_sequence:
                raise ConversationJournalError("editor input history contains a sequence gap")
            if previous.ends_mid_turn != following.starts_mid_turn:
                raise ConversationJournalError("editor input turn-boundary flags are inconsistent")
        messages = tuple(message for segment in selected for message in segment.messages)
    ...
    def _read_ingress(
        ...
        address: ConversationAddress,
        delivery_id: str,
    ) -> ConversationIngressReceipt | None:
        try:
            return self.ingress_receipts.read(address, delivery_id)
        except ConversationIngressError as exc:
            raise ConversationJournalError(str(exc)) from exc

    def _prepare_ingress(
        self,
        address: ConversationAddress,
        ingress: ConversationIngressRequest,
        batch: ConversationBatch,
    ) -> ConversationIngressReceipt:
        try:
            receipt = self.ingress_receipts.prepare(
                address,
                ingress,
            ...
            raise ConversationJournalError(str(exc)) from exc
        self._require_matching_ingress(receipt, ingress, batch)
        return receipt
    ...
    def _commit_ingress(
        ...
        try:
            return self.ingress_receipts.commit(address, receipt)
        except ConversationIngressError as exc:
            raise ConversationJournalError(str(exc)) from exc

    @staticmethod
    def _require_matching_ingress(
        receipt: ConversationIngressReceipt,
        request: ConversationIngressRequest,
        batch: ConversationBatch,
    ) -> None:
        if receipt.delivery_id != request.delivery_id:
            raise ConversationJournalError("ingress receipt identity changed unexpectedly")
        if (
            receipt.request_digest != request.request_digest
            or receipt.batch_digest != batch.digest
            or receipt.start_sequence != batch.start_sequence
            or receipt.end_sequence != batch.end_sequence
        ):
            raise ConversationWriteConflictError("delivery_id is already bound to a different conversation append")

    def _recover_all_prepared_ingress(
        self,
        address: ConversationAddress,
        *,
        ...
        try:
            receipts = self.ingress_receipts.list(address)
        except ConversationIngressError as exc:
            raise ConversationJournalError(str(exc)) from exc
        for receipt in receipts:
            if receipt.state is ConversationIngressState.PREPARED:
                self._recover_ingress_receipt(
                    address,
                    receipt,
                    references=references,
                    live_messages=live_messages,
    ...
    def _recover_ingress_receipt(
        ...
        *,
        ...
        if receipt.state is ConversationIngressState.COMMITTED:
            return receipt
        retained: dict[int, ConversationMessage] = {message.sequence: message for message in live_messages}
        for reference in references:
            if reference.end_sequence < receipt.start_sequence or reference.start_sequence > receipt.end_sequence:
                continue
            for message in self.read_segment(address, reference.segment_id).messages:
                if receipt.start_sequence <= message.sequence <= receipt.end_sequence:
                    retained[message.sequence] = message
        expected_sequences = tuple(range(receipt.start_sequence, receipt.end_sequence + 1))
        selected = tuple(retained[sequence] for sequence in expected_sequences if sequence in retained)
        if not selected:
            current_next = self._expected_next_sequence(state, live_messages)
            if current_next <= receipt.start_sequence:
                try:
                    self.ingress_receipts.discard_prepared(address, receipt)
                except ConversationIngressError as exc:
                    raise ConversationJournalError(str(exc)) from exc
                return None
            raise ConversationJournalError("prepared ingress receipt lost its retained conversation range")
        if len(selected) != len(expected_sequences):
            raise ConversationJournalError("prepared ingress receipt covers a partial conversation range")
        recovered = ConversationBatch(state.conversation_id, selected)
        if recovered.digest != receipt.batch_digest:
            raise ConversationWriteConflictError("prepared ingress receipt conflicts with retained conversation bytes")
        return self._commit_ingress(address, receipt)
    ...
    def _load_state(
        ...
        state = self._read_state_file(address) or ConversationJournalState.empty(self._source_conversation_id(address))
    ...
    def _known_for_append(
        ...
        *,
        ...
        if batch.start_sequence < expected_next and batch.start_sequence <= state.released_through:
            raise ConversationWriteConflictError("append replay predates released conversation history")
        known_by_sequence = {item.sequence: item for item in self._known_tail(latest, live_messages)}
        if batch.start_sequence < expected_next:
            for reference in references:
                if reference.end_sequence < batch.start_sequence or reference.start_sequence >= expected_next:
                    continue
    ...
    def _source_conversation_id(self, address: ConversationAddress) -> str:
        directory = self.layout.conversation_directory(address)
        return directory.name if directory.exists() else address.conversation_id
    ...
    def _batch_or_none(
        ...
        return ConversationBatch(conversation_id, resolved) if resolved else None
