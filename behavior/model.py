...
class BehaviorKind(str, Enum):
    ...
    ``OCCURRENCE`` 是一次被观测到的行为（有目标的事件与无目标的动作段共用一类，靠 ``goal``
    空非空区分）；``GAP`` 是一段观测空白（没读懂 / 未观测），与行为共享同一条时间轴。
...
# - 语义面（判断存储会释放，此处必须自足）：goal（可空=动作段，照常进树）、summary（链内有序拼接）、
...
#
# ── 日循环边界与上游缺口 ─────────────────────────────────────────────────────────────────
# - 树滞后约一个融合回看窗（现 1h，唯一出处 behavior/fusion/config.py）：**当日实况（今天做了
#   什么、门控键值）永不读树**，实时来源是判断存储——判断存储因此有两个消费者（归约缓冲 + 在线
#   实况），释放门槛必须等两者（BHV-LIFECYCLE-001 的新输入）。夜批读到的"昨天"缺傍晚尾巴的处理（推迟批时点 or 连未封缓冲一起读）见待定。
