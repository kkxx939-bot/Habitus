...
一条判断对应主体的**一个行为单位**——原则上可以对他说"该做 X 了"、或者替他把 X 做掉的
那种事：吃饭、做早餐、和人交谈、给设备充电、洗餐具、锁门、喝水、出门。

  behavior 有             →  一件事；goal 有就写、说不出就留空（锁门、喝水常说不出目标，照样是一件事）
  behavior 空             →  这段观测没读懂      （其余字段全部留空）

不是一件事的两种东西：
  **步骤**——拿起、放下、走到桌前、拔下充电线、瞥一眼手机、中途洗手：它们是某件事的一步，
           写进那件事的 basis，不单独立条。
...
【最重要】frames 必须**逐行覆盖输入的每一条片段**，一行都不能少，顺序与输入完全
          ...
          每一行怎么填：属于某件事或它的步骤 → 写归属；读不懂 → 归给一条 behavior 为空的
          判断；无意识小动作、过渡帧 → 填 []。位移帧先问它服务于哪件事："走向厨房"通常是
          做饭的第一步；两件事之间漫无目的地走动才是过渡。
...
不是把八条"人在搓手"抄一遍。说不出目标的单位（喝水：拿杯、倒水、喝）同样可以写步骤；
分不出步骤就填 []。
...
要点：
  ...
  #7 是**别人**在做的事，另起一条判断、subjects 写家庭成员B，**不要**因为它夹在洗手中间
      ...
      是正常的。
  #10 打哈欠是无意识的小动作，不是一件事也不是谁的步骤：frames 填 []，judgements 里不出现。
  #11 读不懂，单独一条 behavior 为空的判断，**不要丢弃**——读不懂和"不构成事"是两回事。
...
judgements：
  ...
  2  subjects=[家庭成员A]  behavior=看手机  goal=查看手机内容  summary=边吃饭边看手机
     ...
     basis: 1「拿起手机注视屏幕」
  3  subjects=[家庭成员A]  behavior=继续吃饭  goal=吃完这顿饭  summary=回到餐桌继续吃
     status=ongoing  status_basis=observation_lost
...
要点：
  ...
  #7 起身走开——不知道去做什么，也不是吃饭的一步：过渡帧，填 []，不单独立条。
...
judgements：
  1  subjects=[家庭成员A]  behavior=做早餐  goal=做煎蛋  summary=取蛋磕蛋、洗手、搅打蛋液并取芝士备用
     status=ongoing  status_basis=observation_lost  relations=[]
     basis: 1「从冰箱和橱柜取出鸡蛋和碗，磕蛋入碗，扔掉蛋壳」 2「中途冲洗擦干双手」
            3「搅打蛋液」 4「查看冷冻室并取出芝士」
