...
def test_a_segment_with_nothing_remindable_may_yield_no_judgement() -> None:
    """一段只有晃身体、扶眼镜、点头的观测里没有一件能提醒的事：全部帧无归属、零判断是合法产出。

    曾在这里硬失败要求"至少声明一条"，真实切片上模型被打回后只会把整段改判成"读不懂"——
    读得懂却不构成事，正是无归属要表达的。
    """

    batch = checked(wire([], [[], [], [], [], []]))
    assert batch.judgements == ()
    assert batch.unowned_fragment_nos == (1, 2, 3, 4, 5)
