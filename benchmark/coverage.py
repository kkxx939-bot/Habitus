...
def benchmark_suite_matrix() -> tuple[Mapping[str, object], ...]:
    ...
    return (
        ...
        _suite("retrieval_performance", "implemented", "并发 SearchService p50/p95/p99 与 QPS"),
        _suite("vector_backend", "implemented", "真实 Embedder/VectorStore 全量、过滤检索、增量更新删除"),
        _suite("runtime_contention", "implemented", "Conversation commit、MemoryJob 与 SearchService 混合负载"),
        _suite("conversation_lifecycle", "implemented", "Segment、Range、Archive、History/Job/Receipt 清理"),
        _suite("memory_semantic_evolution", "implemented", "仓库 native 数据覆盖六类节点、更新删除合并和关系"),
        _suite("failure_recovery", "implemented", "真实主链注入一次瞬态 VectorStore 故障并测量耐久恢复"),
