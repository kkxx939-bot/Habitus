...
class PublishedVectorStore:
    ...
    def __init__(self, backend: RawVectorBackend) -> None:
    ...
    async def read(self, identities: tuple[str, ...]) -> tuple[VectorStoreRecord, ...]:
        ...
        if not normalized:
            return ()
        await self.initialize()
        records = tuple(await self.backend.read(normalized))
    ...
    async def _load_publication(self) -> VectorPublicationSnapshot:
        ...
        return VectorPublicationSnapshot(
            state=None if state_value is None else _state_from_metadata(state_value),
            claim_exists=claim_value is not None,
            building=False if claim_value is None else _claim_from_metadata(claim_value),
        )
...
def _claim_from_metadata(value: Mapping[str, object]) -> bool:
    if set(value) != {"format", "building"} or value.get("format") != _PUBLICATION_FORMAT:
    ...
    if publication.state is None:
        if publication.claim_exists and not publication.building:
            raise VectorStoreIntegrityError(
                f"vector ownership claim is ready without state during {operation}"
            )
        return
