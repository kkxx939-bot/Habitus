...
from infrastructure.store.filesystem import DurablePathIntegrityError, list_real_directory
...
class ConversationAddress:
    ...
    started_on: date
    _identity: str = field(init=False, repr=False, default="")
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "_identity",
            canonical_path_identity(identifier, "conversation_id"),
        )
...
class ConversationLayout:
    ...
    def __init__(
        ...
        root: str | Path,
        *,
        max_conversation_tree_entries: int,
    ) -> None:
        if (
            isinstance(max_conversation_tree_entries, bool)
            or not isinstance(max_conversation_tree_entries, int)
            or max_conversation_tree_entries <= 0
        ):
            raise ValueError("max_conversation_tree_entries must be a positive integer")
        ...
        self.max_conversation_tree_entries = max_conversation_tree_entries
    ...
    def conversation_directory(self, address: ConversationAddress) -> Path:
        ...
        return self._inside_root(self._existing_identity_directory(parent, resolved.identity))
    ...
    def ingress_directory(self, address: ConversationAddress) -> Path:
        """返回独立于 History 保留期的写入幂等回执目录。"""

        return self._inside_root(self.conversation_directory(address) / "ingress")

    def ingress_path(self, address: ConversationAddress, delivery_id: str) -> Path:
        if not isinstance(delivery_id, str) or re.fullmatch(r"[0-9a-f]{64}", delivery_id) is None:
            raise ConversationLayoutError("delivery_id must be lowercase SHA-256 text")
        return self._inside_root(self.ingress_directory(address) / f"{delivery_id}.json")
    ...
    def lock_key(self, address: ConversationAddress) -> str:
        resolved = self._address(address)
        return f"conversation:{self._root_key}:{resolved.started_on.isoformat()}:{resolved.identity}"

    def _existing_identity_directory(self, parent: Path, identity: str) -> Path:
        try:
            entries = list_real_directory(
                parent,
                artifact_root=self.root,
                max_entries=self.max_conversation_tree_entries,
            )
        except DurablePathIntegrityError as exc:
            raise ConversationLayoutError("conversation identity directory cannot be resolved safely") from exc
        matches = []
        for entry in entries:
            try:
                entry_identity = canonical_path_identity(
                    entry.name,
        ...
        if len(matches) > 1:
            raise ConversationLayoutError("conversation tree contains multiple physical aliases for one identity")
        if not matches:
            return parent / identity
        if not matches[0].is_dir():
            raise ConversationLayoutError("conversation identity path is not a directory")
        return parent / matches[0].name
