...
_VISIBILITY_SCHEMA = "memory_commit_visibility_v1"
_VISIBILITY_FILE = ".visibility.json"
...
class MemoryTransactionJournal:
    ...
    def pending(self) -> tuple[MemoryTransactionJournalRecord, ...]:
        ...
        for child in self.root.iterdir():
            ...
            if child.name == _VISIBILITY_FILE or temporary_destination == _VISIBILITY_FILE:
                continue
    ...
    def visibility_generation(self) -> int:
        ...
        try:
            ...
            raise MemoryTransactionJournalError("transaction visibility generation is invalid") from exc
        if (
            not isinstance(value, Mapping)
            or set(value) != {"schema", "generation"}
            or value.get("schema") != _VISIBILITY_SCHEMA
        ):
            raise MemoryTransactionJournalError("transaction visibility generation has an invalid shape")
        generation = value.get("generation")
        if isinstance(generation, bool) or not isinstance(generation, int) or generation < 0:
            raise MemoryTransactionJournalError("transaction visibility generation must be non-negative")
        return generation

    def advance_visibility_generation(self) -> int:
        """在事务全局租约内原子推进读侧提交边界。"""

        generation = self.visibility_generation() + 1
        try:
            atomic_replace_bytes(
                self.root / _VISIBILITY_FILE,
                canonical_json(
                    {
                        "schema": _VISIBILITY_SCHEMA,
            ...
            raise MemoryTransactionJournalError("failed to advance transaction visibility generation") from exc
        return generation
    ...
    def discard_terminal(self, transaction_id: str) -> None:
        ...
        path = self._path(transaction_id)
        try:
            if not durable_unlink(path, artifact_root=self.root):
